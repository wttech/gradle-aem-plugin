...
class InstanceSync(aem: AemExtension, instance: Instance) : InstanceHttpClient(aem, instance) {
    ...
    fun installPackage(remotePath: String, recursive: Boolean = true, retry: Retry = aem.retry()): InstallResponse {
        return retry.withCountdown<InstallResponse, InstanceException>("install package") {
            ...
            val response = try {
                postMultipart(url, mapOf("recursive" to recursive)) { InstallResponse.from(asStream(it), aem.packageOptions.responseBuffer) }
            } catch (e: RequestException) {
    ...
    fun deletePackage(remotePath: String): DeleteResponse {
        ...
        val response = try {
            postMultipart(url) { DeleteResponse.from(asStream(it), aem.packageOptions.responseBuffer) }
        } catch (e: RequestException) {
    ...
    fun uninstallPackage(remotePath: String): UninstallResponse {
        ...
        val response = try {
            postMultipart(url) { UninstallResponse.from(asStream(it), aem.packageOptions.responseBuffer) }
        } catch (e: RequestException) {
