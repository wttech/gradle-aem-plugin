...
class StackGesellix(private val aem: AemExtension) : Stack {
    ...
    override fun rm() = docker { stackRm(options.stackName) }
    ...
    private fun DockerClient.initSwarmIfNotInitialized() {
        try {
            aem.logger.info("Swarm already initialized: $swarmManagerToken")
        } catch (dce: DockerClientException) {
            val cause = dce.cause
            if (cause is IllegalStateException && cause.message == "docker swarm inspect failed") {
                initSwarm()
                aem.logger.info("Swarm initialized")
            } else {
