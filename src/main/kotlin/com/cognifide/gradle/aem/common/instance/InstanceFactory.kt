...
class InstanceFactory(val aem: AemExtension) {
    ...
    fun createByUrl(httpUrl: String, options: Instance.() -> Unit = {}) {
        val parsed = InstanceUrl.parse(httpUrl)
        create(parsed.name, options).apply { this.httpUrl.set(parsed.httpUrl) }
    }
    ...
    fun remoteByUrl(httpUrl: String, options: Instance.() -> Unit = {}) {
        val parsed = InstanceUrl.parse(httpUrl)
        remote(parsed.name, options).apply { this.httpUrl.set(parsed.httpUrl) }
    }
    ...
    fun localByUrl(httpUrl: String, options: LocalInstance.() -> Unit = {}) {
        val parsed = InstanceUrl.parse(httpUrl)
        local(parsed.name, options).apply { this.httpUrl.set(parsed.httpUrl) }
    }
