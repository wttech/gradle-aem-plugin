...
class FilterFile(@InputFile val file: File, val temporary: Boolean = false) : Closeable {
    ...
    companion object {
        ...
        fun default(sling: SlingExtension): FilterFile = cmd(sling) ?: convention(sling)

        fun cmd(sling: SlingExtension): FilterFile? = sling.run {
        ...
        fun convention(sling: SlingExtension): FilterFile = sling.run {
            val dir = packageOptions.vltDir.get().asFile
        ...
        fun temporary(sling: SlingExtension, paths: List<String>): FilterFile {
            val template = sling.assetManager.readFile("vlt/$TEMPORARY_NAME").bufferedReader().use { it.readText() }
            val content = sling.prop.expand(template, mapOf("paths" to paths))
            val file = sling.common.temporaryFile("${PackageVlt.NAME}/$TEMPORARY_NAME")
