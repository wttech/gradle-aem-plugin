...
class HelpCheck(group: CheckGroup) : DefaultCheck(group) {
     ...
     */
    val enabled = sling.obj.boolean { convention(true) }

    /**
     * After longer inactivity time, try helping instance going back to healthy state.
     */
    val stateTime = sling.obj.long { convention(TimeUnit.MINUTES.toMillis(8)) }

    /**
     * Bundle with these states are considered for forcing start.
     */
    val bundleStartStates = sling.obj.strings {
        convention(listOf(
     ...
     */
    val bundleStartDelay = sling.obj.long { convention(TimeUnit.SECONDS.toMillis(3)) }
