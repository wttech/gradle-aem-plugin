...
import com.cognifide.gradle.sling.common.pkg.vault.VaultException
import java.io.File
...
class Cleaner(private val sling: SlingExtension) {
    ...
    /**
     * Properties that will be skipped when pulling JCR content from Sling instance.
     *
     ...
     */
    val propertiesSkipped = sling.obj.strings {
        convention(listOf(
    ...
    /**
     * Mixin types that will be skipped when pulling JCR content from Sling instance.
     */
    val mixinTypesSkipped = sling.obj.strings {
        convention(listOf(
                "mix:versionable"
        ))
    }
     ...
     */
    val namespacesSkipped = sling.obj.boolean {
        convention(true)
        sling.prop.boolean("package.sync.cleaner.namespacesSkipped")?.let { set(it) }
    }

    /**
     * Controls backups for parent nodes of filter roots for keeping them untouched.
     */
    val parentsBackupEnabled = sling.obj.boolean {
        convention(true)
        sling.prop.boolean("package.sync.cleaner.parentsBackup")?.let { set(it) }
    }
     ...
     */
    val parentsBackupSuffix = sling.obj.string { convention(".bak") }
    ...
    private fun eachFiles(root: File, filter: PatternFilterable.() -> Unit, action: (File) -> Unit) {
        ...
        sling.project.fileTree(root.parent).matching(rootFilter).matching(filter).forEach(action)
    }
    ...
    private fun cleanDotContentFile(file: File) {
        ...
        try {
            sling.logger.info("Cleaning file {}", file.path)

            val inputLines = FileUtils.readLines(file, StandardCharsets.UTF_8.name())
            val filteredLines = filterLines(file, inputLines)

            FileUtils.writeLines(file, StandardCharsets.UTF_8.name(), filteredLines, sling.commonOptions.lineSeparator.get().value)
        } catch (e: IOException) {
    ...
    private fun flattenFile(file: File) {
        ...
        if (dest.exists()) {
            sling.logger.info("Overriding file by flattening $file")
            FileUtils.deleteQuietly(dest)
        } else {
            sling.logger.info("Flattening file $file")
        }
    ...
    private fun deleteFile(file: File) {
        ...
        sling.logger.info("Deleting file {}", file.path)
    ...
    private fun deleteEmptyDirs(root: File) {
        ...
        if (EmptyFileFilter.EMPTY.accept(root)) {
            sling.logger.info("Deleting empty directory {}", root.path)
            ...
            if (File(parent, parentsBackupDirIndicator.get()).createNewFile()) {
                siblingFiles.filter { !it.name.endsWith(parentsBackupSuffix.get()) }
                        .forEach { origin ->
                            ...
                            sling.logger.info("Doing backup of parent file: $origin")
    ...
    private fun doRootBackup(root: File) {
        if (root.isFile) {
            ...
            sling.logger.info("Doing backup of root file: $root")
        ...
        eachFiles(root, filesFlattened) { file ->
            ...
            sling.logger.info("Doing backup of file: $file")
        ...
        eachParentFiles(root) { _, siblingFiles ->
            if (siblingFiles.any { it.name == parentsBackupDirIndicator.get() }) {
                ...
                siblingFiles.filter { it.name.endsWith(parentsBackupSuffix.get()) }.forEach { backup ->
                    ...
                    sling.logger.info("Undoing backup of parent file: $backup")
