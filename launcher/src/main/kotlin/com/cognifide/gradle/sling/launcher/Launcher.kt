    ...
    companion object {
        ...
        fun main(args: Array<String>) {
            ...
            val pluginVersion = buildConfig["pluginVersion"]?.toString()
                    ?: throw LauncherException("Sling Plugin version info not available!")
        ...
        private fun saveBuildScript(workDir: File, pluginVersion: String?) {
            workDir.resolve("build.gradle.kts").apply {
                ...
                writeText("""
                            plugins {
                                id("com.cognifide.sling.instance.local") version "$pluginVersion"
                                id("com.cognifide.sling.package.sync") version "$pluginVersion"
                            }
                        """.trimIndent())
            }
        }
